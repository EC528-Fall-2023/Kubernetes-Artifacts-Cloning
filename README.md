...
---
## Instructions to use this plugin in a Linux Environment:
## 1. Download
...
## 2. Use it
### Step 1: Have kubeconfig files for both source and destination clusters.
...
### Step 2: Run commands based on different scenarios:
...
```
#### Scenario 2: Clone specific object type(s) using flag: -o. For example, you want to clone object types of pod and service. After the -o, you enter pod,service. Seperate object types using "," and no space between each object type):
```
kubectl clone -s src-config.yaml -d dest-config.yaml -o pod,service
```
#### Scenario 3: Clone objects in a specific namespace using flag: -n. For example, you just want to clone the "default" namespace:
```
kubectl clone -s src-config.yaml -d dest-config.yaml -n default
```
#### Scenario 4: Clone objects that have specific label using flag: -l. For example, you just want to clone a pods that have label of "app=mongo" :
```
